...
public class SparklineChart extends VerticalLayout
{
	...
	private DecimalFormat deciMn = new DecimalFormat("#0.000");

	public SparklineChart(String name, String unit, String prefix, Color color,
			int howManyPoints, int min, int max, double amount, String format, int number)
	{
		//setSizeUndefined();
		addStyleName("spark");
		setMargin(false);
		...
		current = new Label(prefix + (format.equals("Deci")?deciMn.format(amount):number) + unit);
